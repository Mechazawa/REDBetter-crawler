...
def main():
    ...
    try:
        ...
        do_24_bit = config.get('whatcd', '24bit_behaviour')
        ...
        supported_formats = [format.strip().upper() for format in config.get('whatcd', 'formats').split(',')]

        try:
            media_config = config.get('whatcd', 'media')
            if not media_config:
